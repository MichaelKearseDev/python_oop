...
class IceCreamStand(Restaurant):
    ...
    def __init__(self, restaurant_name, cuisine_type, flavors):
        super().__init__(restaurant_name, cuisine_type)  # Passing the inherited parameters
        self.flavors = flavors  # This is specific to IceCreamStand
        
    def flavors_wanted(self):
        if not self.flavors:
            print("Please list flavors!")
        else:
            # Simply print the list of flavors directly
            flavors_formatted = ', '.join(self.flavors)
            print(f"Your choice of flavors are: {flavors_formatted}")
...
order1 = IceCreamStand("Scoop", "Ice Cream", ["Chocolate", "Vanilla", "Maple"])
order1.flavors_wanted()
